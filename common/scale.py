...
class RunningScale(torch.nn.Module):
	...
	def __init__(self, cfg):
		super().__init__()
		self.cfg = cfg
		self.value = Buffer(torch.ones(1, dtype=torch.float32, device=torch.device('cuda:1')))
		self._percentiles = Buffer(torch.tensor([5, 95], dtype=torch.float32, device=torch.device('cuda:1')))
...
class Harmonizer(torch.nn.Module):
	def __init__(self, enable=True):
		super().__init__()
		self.harmony_s = torch.nn.Parameter(torch.tensor(0.0))
		self.enable = enable

	def forward(self, loss):
		if self.enable:
			scale = torch.exp(self.harmony_s)
			return loss / scale + torch.log(torch.exp(scale) + 1)
		else:
			return loss
